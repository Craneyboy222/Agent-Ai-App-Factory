...
export default function Research() {
  ...
  async function runResearch() {
    ...
    try {
      ...
      // Use a relative URL when no base URL is specified to avoid CORS issues.
      const url = baseUrl ? `${baseUrl}/api/research` : '/api/research';
      const res = await axios.post(url);
      setIdeas(res.data.ideas);
    } catch (err: any) {
      console.error(err);
      // Include the network error message if present so users aren't left with a generic error.
      setError(err?.response?.data?.message || err?.message || 'Failed to run market research');
    } finally {
      setLoading(false);
    }
  }

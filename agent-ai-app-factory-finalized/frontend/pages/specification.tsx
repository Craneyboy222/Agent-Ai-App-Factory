...
export default function SpecificationPage() {
  ...
  useEffect(() => {
    async function fetchSpec() {
      ...
      try {
        ...
        const res = await axios.post(`${baseUrl}/api/specification`, { idea });
        setSpec(res.data);
      } catch (err: any) {
        console.error(err);
        setError(err?.response?.data?.message || 'Failed to generate specification');
      } finally {
        setLoading(false);
      }
    }
  ...
  // Encode specification as base64 JSON for passing to the next step
  const encodedSpec = spec ? Buffer.from(JSON.stringify(spec)).toString('base64') : '';
